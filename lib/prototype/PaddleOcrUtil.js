 ...
 */
 PaddleOcrUtil.prototype.ensureModelLoaded = function () {
  let _this = this
  let ocrResults = bridges.toArray(_this.predictor.runOcr(images.fromBase64(check_model_base64).getBitmap()))
  ...
  threads.start(function () {
    while (checkResult != '测试' && limit-- > 0) {
      ...
      ocrResults = bridges.toArray(_this.predictor.runOcr(images.fromBase64(check_model_base64).getBitmap()))
...
PaddleOcrUtil.prototype.recognize = function (img, region) {
  ...
  if (this.enabled) {
    ...
    start = new Date()
    let ocrResults = bridges.toArray(this.predictor.runOcr(bitmap)).filter(v => v.confidence > 0.5)
...
PaddleOcrUtil.prototype.recognizeWithBounds = function (img, region, regex, showOcrDetail) {
  ...
  if (this.enabled) {
    ...
    LogUtils.debugInfo(['图片转换耗时：{}ms', new Date() - start])
    start = new Date()
    let result = bridges.toArray(this.predictor.runOcr(bitmap)).filter(v => v.confidence > 0.5)
    LogUtils.debugInfo(['paddle识别文本耗时：{}ms', new Date() - start])
    if (showOcrDetail) {
