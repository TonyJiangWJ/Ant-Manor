 ...
 * @Last Modified time: 2020-01-12 20:29:40
 * @Description: 手机解锁模块
 */
...
function Unlocker() {
  const _device = new MyDevice(this),
    _HEIGHT = config.device_height,
    _WIDTH = config.device_width,
  ...
  this.failed = function () {
    ...
    this.reTry++
    if (this.reTry > 3) {
      logInfo('解锁失败达到三次，停止运行：' + config.password)
      device.setBrightnessMode(1)
      throw new java.lang.Exception('解锁失败达到三次')
    } else {
