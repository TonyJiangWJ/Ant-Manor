 ...
 * @Last Modified time: 2019-11-29 17:39:41
 * @Description: 通用方法
 */
...
function CommonFunctions () {
  ...
  this.checkCaptureScreenPermission = function (releaseLock, errorLimit) {
    ...
    } while (!screen && errorCount < (errorLimit || 3))
    if (!screen) {
  ...
  this.getSleepTimeAutoCount = function () {
    ...
    let returnVal = sleepStorage.sleepTime || 10
    sleepStorage.count = (sleepStorage.count || 0) + 1
    if (sleepStorage.count <= 4 && returnVal !== 5) {
      _debugInfo(['校验小偷鸡，睡眠5分钟 count:{}', sleepStorage.count])
      sleepStorage.sleepTime = 5
    } else if (sleepStorage.count > 4 && returnVal <= 10) {
      sleepStorage.sleepTime = 20
      _debugInfo('10分钟循环已达到4次以上，修改睡眠时间为20分钟')
    } else if (returnVal > 200) {
      sleepStorage.sleepTime = 20
      _debugInfo('已经睡眠过较长时间，修改睡眠时间为20分钟')
    }
  ...
  this.getTodaysRuntimeStorage = function (key) {
    ...
    _debugInfo('获取缓存数据：' + JSON.stringify(returnVal))
    return returnVal
  }
