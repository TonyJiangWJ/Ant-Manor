...
const ProjectCommonFunction = function () {
  ...
  this.keyList = [SLEEP_TIME, VILLAGE_SPEED_UP]
  ...
  this.getSleepTimeByOcr = function (restTime) {
    ...
    NotificationHelper.createNotification((punched || passedTime > 40) ? '等待小鸡吃完' : '循环驱赶野鸡', 
      util.format('喂食周期时间：%d 喂食后经过时间：%d 下次执行时间：%s', fullTime, passedTime, formatDate(new Date(new Date().getTime() + returnVal * 60000))))
    return returnVal
  }
...
ProjectCommonFunction.prototype.initStorageFactory = function () {
  ...
  storageFactory.initFactoryByKey(VILLAGE_SPEED_UP, {
    collected: false
  })
}
