...
2. **授权与设置**:
    ...
    * (可选但推荐操作) [通过 ADB 授权脚本自动获取无障碍权限](https://github.com/TonyJiangWJ/AutoScriptBase/blob/master/resources/doc/ADB%E6%8E%88%E6%9D%83%E8%84%9A%E6%9C%AC%E8%87%AA%E5%8A%A8%E5%BC%80%E5%90%AF%E6%97%A0%E9%9A%9C%E7%A2%8D%E6%9D%83%E9%99%90.md)。
3. **首次配置**:
    * **重要**: 由于设备差异，默认配置可能无法直接使用，请务必运行 `可视化配置.js` 进行个性化设置。强烈建议安装支持YOLO的版本。
...
* **YOLO 目标检测 (推荐)**:
  ...
  * 在 `执行配置` 中，可以选择不驱赶好友的小鸡，同时可以开启自动雇佣好友的小鸡。推荐饲料没地方去了的时候开启，产蛋速度会有一定程度加快，当前每日任务的饲料基本吃不完。
* **颜色与区域识别 (手动)**:
...
* **定时任务**: 建议设置在当天第一个喂食周期内（如早上9点），以防遗漏。脚本会自动判断是否需要进行下一时段投喂并创建临时定时任务。
* **注意**: 请确保家庭美食数量足够，否则脚本执行可能出错。当美食不足时，可以改用小号投喂，见下方：**小号家庭投喂**
...
* **小号家庭投喂**：
  * 脚本：`unit/家庭投喂-使用小号投喂.js`
  * 功能：当大号的美食数量不足时，可以改用小号来投喂。
  * 定时：建议每天九点多执行，后续将自动创建中午和下午的定时任务。
* **蚂蚁新村大小号助力**:
...
* **脚本**: `星星球.js` 或者 `星星球-YOLO.js` 后者需要运行 `独立工具/星星球模型下载.js` 下载对应的模型文件，对 `星星球-YOLO.js` 创建定时任务可以实现每天自动执行星星球。
