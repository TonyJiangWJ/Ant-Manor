 ...
 * @Last Modified time: 2020-12-26 17:16:15
 * @Description: 
 */
"ui";
...
let bridgeHandler = {
    ...
    ui.run(function () {
      confirm('确定要将所有配置重置为默认值吗？').then(ok => {
        if (ok) {
          ...
          postMessageToWebView({ functionName: 'reloadColorRegionConfigs' })
        }
      })
    })
  },
  exportConfigs: () => {
    ...
    postMessageToWebView({ functionName: 'saveColorRegionConfigs' })
    ui.run(function () {
